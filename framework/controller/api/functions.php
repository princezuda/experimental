...
/*
 allows you to simply define the request or variable and ignore the rest. 
*/
 function sanitizeit($function='')
 {
 $function = '';
 if(empty($function))
<<<<<<< HEAD
  __('this can not be empty');
  if(!empty($function))
   {
...
$allowed_protocols = array('http','https');
 wp_kses($function,$allowed_html,$allowed_protocols);
 }
 }
/**
...
function add_query_arg() {
	...
	$ret = rtrim($ret, '?' ); // sanitizeit is a shortcut function for wp_kses that only allows http and https as protocols and does not want html to get through. 
	return sanitizeit($ret);
}
...
function status_header( $code ) {
	...
	$protocol = esc_attr(wp_unslash($_SERVER['SERVER_PROTOCOL']));
...
>>>>>>> 1e42eb8bfaf9e5c2a55568afd1b7a7087557f4f0
...
function status_header( $code ) {
	...
	$protocol = $_SERVER['SERVER_PROTOCOL']; // fix this
	if ( 'HTTP/1.1' != $protocol && 'HTTP/1.0' != $protocol )
		$protocol = 'HTTP/1.0';
	$status_header = "$protocol $code $description";
	if ( function_exists( 'apply_filters' ) )
...
function wp_original_referer_field( $echo = true, $jump_back_to = 'current' ) {
	if ( ! $ref = wp_get_original_referer() ) {
		$ref = 'previous' == $jump_back_to ? wp_get_referer() : esc_attr(wp_unslash( $_SERVER['REQUEST_URI']) );
	}
...
function iis7_supports_permalinks() {
	...
	if ( $is_iis7 ) {
	/*	 First we check if the DOMDocument class exists. If it does not exist, then we bail out and tell user that
...
function wp_allowed_protocols() {
	...
	if ( empty( $protocols ) ) {
		$protocols = array( 'http', 'https', 'ftp', 'ftps', 'mailto', 'news', 'irc', 'nntp', 'feed', 'telnet', 'mms', 'rtsp', 'svn', 'tel', 'fax', 'xmpp' );
